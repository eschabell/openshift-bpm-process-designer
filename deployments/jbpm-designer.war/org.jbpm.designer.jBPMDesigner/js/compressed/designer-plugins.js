...
if(ORYX.PRESET_PERSPECTIVE.length>0){if(ORYX.PRESET_PERSPECTIVE=="full"){this._updateStencil("http://b3mn.org/stencilset/bpmn2.0#")
}else{if(ORYX.PRESET_PERSPECTIVE=="simple"){this._updateStencil("http://b3mn.org/stencilset/bpmn2.0simple#")
}else{if(ORYX.PRESET_PERSPECTIVE=="ruleflow"){this._updateStencil("http://b3mn.org/stencilset/bpmn2.0ruleflow#")
...
},_updateStencil:function(d){var c=this.facade.getStencilSets();
var b=new Object();
...
var a={};
if(("webkitSpeech" in document.createElement("input"))){var d=new Ext.form.TextField({id:"micinput"});
this.toolbar.add(d);
this.toolbar.add("-");
...
this.facade.offer({name:ORYX.I18N.View.migratejPDL,functionality:this.migrateJPDL.bind(this),group:"importgroup",icon:ORYX.BASE_FILE_PATH+"images/jpdl_import_icon.png",dropDownGroupIcon:ORYX.BASE_FILE_PATH+"images/import.png",description:ORYX.I18N.View.migratejPDLDesc,index:3,minShape:0,maxShape:0,isEnabled:function(){return true
}.bind(this)})
...
this.facade.offer({name:ORYX.I18N.View.connectServiceRepo,functionality:this.jbpmServiceRepoConnect.bind(this),group:"servicerepogroup",icon:ORYX.BASE_FILE_PATH+"images/repository_rep.gif",description:ORYX.I18N.View.connectServiceRepoDesc,index:4,minShape:0,maxShape:0,isEnabled:function(){return true
}.bind(this)})
...
this.repoContent=new Ext.TabPanel({activeTab:0,border:false,width:"100%",height:"100%",tabPosition:"top",layoutOnTabChange:true,deferredRender:false,defaults:{autoHeight:true,autoScroll:true},items:[{title:"Service Nodes",anchor:"100%",autoScroll:true,layout:"table",items:[d],margins:"10 10 10 10",listeners:{tabchange:function(j,i){tabs.doLayout()
}}}]});
this.repoDialog.add(this.repoContent);
this.repoDialog.doLayout()
...
}},showMorphButton:function(c){if(c.length!=1){return
}var a=this.facade.getRules().morphStencils({stencil:c[0].getStencil()});
a=a.select(function(d){if(c[0].getStencil().type()==="node"){return this.facade.getRules().canContain({containingShape:c[0].parent,containedStencil:d})
}else{return this.facade.getRules().canConnect({sourceShape:c[0].dockers.first().getDockedShape(),edgeStencil:d,targetShape:c[0].dockers.last().getDockedShape()})
}}.bind(this));
if(a.size()<=1){return
}this.morphMenu.removeAll();
if(c[0].getStencil().id().endsWith("#Task")){if(c[0].properties["oryx-tasktype"]!="User"){var b=new Ext.menu.Item({text:"User Task",icon:ORYX.BASE_FILE_PATH+"stencilsets/bpmn2.0jbpm/icons/activity/list/type.user.png",disabled:false,disabledClass:ORYX.CONFIG.MORPHITEM_DISABLED,handler:(function(){this.updateTaskType(c[0],"User")
}).bind(this)});
this.morphMenu.add(b)
}if(c[0].properties["oryx-tasktype"]!="Send"){var b=new Ext.menu.Item({text:"Send Task",icon:ORYX.BASE_FILE_PATH+"stencilsets/bpmn2.0jbpm/icons/activity/list/type.send.png",disabled:false,disabledClass:ORYX.CONFIG.MORPHITEM_DISABLED,handler:(function(){this.updateTaskType(c[0],"Send")
}).bind(this)});
this.morphMenu.add(b)
}if(c[0].properties["oryx-tasktype"]!="Receive"){var b=new Ext.menu.Item({text:"Receive Task",icon:ORYX.BASE_FILE_PATH+"stencilsets/bpmn2.0jbpm/icons/activity/list/type.receive.png",disabled:false,disabledClass:ORYX.CONFIG.MORPHITEM_DISABLED,handler:(function(){this.updateTaskType(c[0],"Receive")
}).bind(this)});
this.morphMenu.add(b)
}if(c[0].properties["oryx-tasktype"]!="Manual"){var b=new Ext.menu.Item({text:"Manual Task",icon:ORYX.BASE_FILE_PATH+"stencilsets/bpmn2.0jbpm/icons/activity/list/type.manual.png",disabled:false,disabledClass:ORYX.CONFIG.MORPHITEM_DISABLED,handler:(function(){this.updateTaskType(c[0],"Manual")
}).bind(this)});
this.morphMenu.add(b)
}if(c[0].properties["oryx-tasktype"]!="Service"){var b=new Ext.menu.Item({text:"Service Task",icon:ORYX.BASE_FILE_PATH+"stencilsets/bpmn2.0jbpm/icons/activity/list/type.service.png",disabled:false,disabledClass:ORYX.CONFIG.MORPHITEM_DISABLED,handler:(function(){this.updateTaskType(c[0],"Service")
}).bind(this)});
this.morphMenu.add(b)
}if(c[0].properties["oryx-tasktype"]!="Business Rule"){var b=new Ext.menu.Item({text:"Business Rule Task",icon:ORYX.BASE_FILE_PATH+"stencilsets/bpmn2.0jbpm/icons/activity/list/type.business.rule.png",disabled:false,disabledClass:ORYX.CONFIG.MORPHITEM_DISABLED,handler:(function(){this.updateTaskType(c[0],"Business Rule")
}).bind(this)});
this.morphMenu.add(b)
}if(c[0].properties["oryx-tasktype"]!="Script"){var b=new Ext.menu.Item({text:"Script Task",icon:ORYX.BASE_FILE_PATH+"stencilsets/bpmn2.0jbpm/icons/activity/list/type.script.png",disabled:false,disabledClass:ORYX.CONFIG.MORPHITEM_DISABLED,handler:(function(){this.updateTaskType(c[0],"Script")
}).bind(this)});
this.morphMenu.add(b)
}}a=a.sortBy(function(d){return d.position()
});
a.each((function(e){if(!(c[0].properties["oryx-nomorph"]&&c[0].properties["oryx-nomorph"]=="true")){var d=new Ext.menu.Item({text:e.title(),icon:e.icon(),disabled:e.id()==c[0].getStencil().id(),disabledClass:ORYX.CONFIG.MORPHITEM_DISABLED,handler:(function(){this.morphShape(c[0],e)
}).bind(this)});
this.morphMenu.add(d)
}}).bind(this));
...
}},onSelectionChanged:function(c){this.grid.stopEditing();
this.shapeSelection.shapes=c.elements;
if(c.elements){if(c.elements.length==0){this.shapeSelection.shapes=[this.facade.getCanvas()]
}else{var b=this.shapeSelection.shapes.length;
while(b--){var a=this.shapeSelection.shapes[b];
if(a&&(a instanceof ORYX.Core.Node||a instanceof ORYX.Core.Edge)&&a.properties["oryx-isselectable"]=="false"){this.shapeSelection.shapes.splice(b)
}}if(this.shapeSelection.shapes.length==0){this.shapeSelection.shapes=[this.facade.getCanvas()]
...
var y=[];
v.each(function(i){if(i.value()==C){C=i.title()
...
var m=new Ext.Panel({id:"calledElementsPanel",title:'<center>Select Process Id and click "Save" to select.</center>',layout:"column",items:[g],layoutConfig:{columns:1},defaults:{columnWidth:1}});
var k=new Ext.Window({layout:"anchor",autoCreate:true,title:"Editor for Called Elements",height:350,width:680,modal:true,collapsible:false,fixedcenter:true,shadow:true,resizable:true,proxyDrag:true,autoScroll:true,keys:[{key:27,fn:function(){k.hide()
...
}}catch(j){this.facade.raiseEvent({type:ORYX.CONFIG.EVENT_NOTIFICATION_SHOW,ntype:"error",msg:"Error resolving other process info :\n"+j,title:""})
...
this.facade.offer({name:"Show in full screen",functionality:function(c){var d=parent.document.getElementById(ORYX.EDITORID);
...
}}}}.bind(this),group:"fullscreengroup",icon:ORYX.BASE_FILE_PATH+"images/fullscreen.png",description:"Show in full screen mode",index:2,minShape:0,maxShape:0,isEnabled:function(){return true
}.bind(this)});
...
this.diffDialog.doLayout()
}},_loadJSON:function(a,b){if(a){Ext.MessageBox.confirm("Import","Replace existing model?",function(c){if(c=="yes"){this.facade.setSelection(this.facade.getCanvas().getChildShapes(true));
...
this.facade.raiseEvent({type:ORYX.CONFIG.EVENT_NOTIFICATION_SHOW,ntype:"success",msg:"Successfully imported "+b,title:""})
}catch(e){this.facade.importJSON(h);
this.facade.raiseEvent({type:ORYX.CONFIG.EVENT_NOTIFICATION_SHOW,ntype:"error",msg:"Unable to import provided "+b,title:""})
...
this.facade.offer({name:ORYX.I18N.Undo.undo,description:ORYX.I18N.Undo.undoDesc,icon:ORYX.BASE_FILE_PATH+"images/arrow_undo.png",keyCodes:[{metaKeys:[ORYX.CONFIG.META_KEY_META_CTRL],keyCode:90,keyAction:ORYX.CONFIG.KEY_ACTION_DOWN}],functionality:this.doUndo.bind(this),group:ORYX.I18N.Undo.group,isEnabled:function(){return this.undoStack.length>0
}.bind(this),index:0});
...
this.facade.offer({name:ORYX.I18N.Grouping.group,functionality:this.createGroup.bind(this),group:ORYX.I18N.Grouping.grouping,icon:ORYX.BASE_FILE_PATH+"images/shape_group.png",description:ORYX.I18N.Grouping.groupDesc,index:1,minShape:2,isEnabled:this.isEnabled.bind(this,false)});
...
this.facade.offer({name:ORYX.I18N.AddDocker.add,functionality:this.enableAddDocker.bind(this),group:ORYX.I18N.AddDocker.group,icon:ORYX.BASE_FILE_PATH+"images/vector_add.png",description:ORYX.I18N.AddDocker.addDesc,index:1,toggle:true,minShape:0,maxShape:0});
...
this.facade.offer({name:ORYX.I18N.Edit.cut,description:ORYX.I18N.Edit.cutDesc,icon:ORYX.BASE_FILE_PATH+"images/cut.png",keyCodes:[{metaKeys:[ORYX.CONFIG.META_KEY_META_CTRL],keyCode:88,keyAction:ORYX.CONFIG.KEY_ACTION_DOWN}],functionality:this.callEdit.bind(this,this.editCut),group:ORYX.I18N.Edit.group,index:1,minShape:1});
...
this.facade.offer({name:"Start validating",functionality:this.enableValidation.bind(this),group:"validationandsimulation",dropDownGroupIcon:ORYX.BASE_FILE_PATH+"images/visualvalidation.png",description:ORYX.I18N.SyntaxChecker.desc,index:1,minShape:0,maxShape:0,isEnabled:function(){return !ORYX.IS_VALIDATING_PROCESS
}});
this.facade.offer({name:"Stop validating",functionality:this.disableValidation.bind(this),group:"validationandsimulation",dropDownGroupIcon:ORYX.BASE_FILE_PATH+"images/visualvalidation.png",description:ORYX.I18N.SyntaxChecker.desc,index:2,minShape:0,maxShape:0,isEnabled:function(){return ORYX.IS_VALIDATING_PROCESS
}});
this.facade.offer({name:"View all issues",functionality:this.viewAllValidation.bind(this),group:"validationandsimulation",dropDownGroupIcon:ORYX.BASE_FILE_PATH+"images/visualvalidation.png",description:ORYX.I18N.SyntaxChecker.desc,index:3,minShape:0,maxShape:0});
...
this.facade.raiseEvent({type:ORYX.CONFIG.EVENT_STENCIL_SET_LOADED});
this.facade.raiseEvent({type:ORYX.CONFIG.EVENT_NOTIFICATION_SHOW,ntype:"info",msg:"Starting continuous visual validation.",title:""});
this.vt=window.setInterval((function(){this.startValidate(true)
}).bind(this),3000)
},disableValidation:function(){ORYX.IS_VALIDATING_PROCESS=false;
this.facade.raiseEvent({type:ORYX.CONFIG.EVENT_STENCIL_SET_LOADED});
this.facade.raiseEvent({type:ORYX.CONFIG.EVENT_NOTIFICATION_SHOW,ntype:"info",msg:"Stopping continuous visual validation.",title:""});
window.clearInterval(this.vt);
this.stopValidate()
...
g=true
}if(g){var e=new Ext.grid.EditorGridPanel({autoScroll:true,autoHeight:true,store:a,stripeRows:true,cm:new Ext.grid.ColumnModel([new Ext.grid.RowNumberer(),{id:"type",header:"Issue Type",width:100,dataIndex:"type",sortable:true,editor:new Ext.form.TextField({allowBlank:true,vtype:"inputName",regex:/^[a-z0-9 \-\.\_]*$/i}),renderer:Ext.util.Format.htmlEncode},{id:"name",header:"Description",width:500,dataIndex:"name",sortable:true,editor:new Ext.form.TextField({allowBlank:true,vtype:"inputName",regex:/^[a-z0-9 \-\.\_]*$/i}),renderer:Ext.util.Format.htmlEncode},{id:"shapeid",header:"Shape ID",width:100,dataIndex:"shapeid",sortable:true,editor:new Ext.form.TextField({allowBlank:true,vtype:"inputName",regex:/^[a-z0-9 \-\.\_]*$/i}),renderer:Ext.util.Format.htmlEncode}]),autoHeight:true,clicksToEdit:1});
var d=new Ext.Window({layout:"anchor",autoCreate:true,title:"Validation Suggestions",height:300,width:700,modal:true,collapsible:false,fixedcenter:true,shadow:true,resizable:true,proxyDrag:true,autoScroll:true,keys:[{key:27,fn:function(){d.hide()
}.bind(this)}],items:[e],listeners:{hide:function(){d.destroy()
}.bind(this)},buttons:[{text:"Close",handler:function(){d.hide()
}.bind(this)}]});
d.show()
}}}});
if(!ORYX.Plugins){ORYX.Plugins=new Object()
...
},chooseFormEditor:function(a){Ext.Msg.show({title:"Form Editor.",msg:"Select which Form Editor to use:",buttons:{yes:"Graphical Modeler",no:"Markup Editor",cancel:"Cancel"},icon:Ext.MessageBox.QUESTION,fn:function(b){if(b=="yes"){this.showTaskFormEditor("form",a)
...
}}catch(h){this.facade.raiseEvent({type:ORYX.CONFIG.EVENT_NOTIFICATION_SHOW,ntype:"error",msg:"Error initiating Form Editor: "+h,title:""})
}}.bind(this),failure:function(){this.facade.raiseEvent({type:ORYX.CONFIG.EVENT_NOTIFICATION_SHOW,ntype:"error",msg:"Error initiating Form Editor",title:""})
},params:{formtype:b,action:"load",taskname:a.tn,profile:ORYX.PROFILE,uuid:ORYX.UUID}})
}catch(f){this.facade.raiseEvent({type:ORYX.CONFIG.EVENT_NOTIFICATION_SHOW,ntype:"error",msg:"Error initiating Form Widgets: "+f,title:""})
}}.bind(this),failure:function(){this.facade.raiseEvent({type:ORYX.CONFIG.EVENT_NOTIFICATION_SHOW,ntype:"error",msg:"Error initiating Form Widgets",title:""})
...
var b=new Ext.form.ComboBox({fieldLabel:"Insert form widget",labelStyle:"width:240px",hiddenName:"widget_name",emptyText:"Insert form widget...",store:m,displayField:"name",valueField:"name",mode:"local",typeAhead:true,triggerAction:"all",listeners:{select:{fn:function(q,i){if(this.taskformcolorsourceeditor){Ext.Ajax.request({url:ORYX.PATH+"formwidget",method:"POST",success:function(r){try{this.taskformcolorsourceeditor.replaceSelection(r.responseText,"end")
}catch(s){this.facade.raiseEvent({type:ORYX.CONFIG.EVENT_NOTIFICATION_SHOW,ntype:"error",msg:"Error inserting Form Widget: "+s,title:""})
}}.bind(this),failure:function(){this.facade.raiseEvent({type:ORYX.CONFIG.EVENT_NOTIFICATION_SHOW,ntype:"error",msg:"Error inserting Form Widget",title:""})
},params:{action:"getwidgetsource",profile:ORYX.PROFILE,widgetname:q.getValue(),uuid:ORYX.UUID}})
}else{this.facade.raiseEvent({type:ORYX.CONFIG.EVENT_NOTIFICATION_SHOW,ntype:"error",msg:"Widget insertion is only possible in Source Mode",title:""})
}}.bind(this)}}});
var j=Ext.id();
...
var p=new Ext.Window({id:"maineditorwindow",layout:"fit",autoCreate:true,title:"Editing Form: "+o+" - Press [Ctrl-Z] to activate auto-completion",height:570,width:930,modal:true,collapsible:false,fixedcenter:true,shadow:true,resizable:true,proxyDrag:true,keys:[{fn:function(){p.close();
p=null
}.bind(this)}],items:[a],listeners:{hide:function(){p=null
}.bind(this)},buttons:[{text:"Save",handler:function(){this.facade.raiseEvent({type:ORYX.CONFIG.EVENT_NOTIFICATION_SHOW,ntype:"info",msg:"Storing Task Form",title:""});
var i="";
...
p=null
}catch(r){this.facade.raiseEvent({type:ORYX.CONFIG.EVENT_NOTIFICATION_SHOW,ntype:"error",msg:"Error saving Task Form: "+r,title:""})
}}.createDelegate(this),failure:function(){this.facade.raiseEvent({type:ORYX.CONFIG.EVENT_NOTIFICATION_SHOW,ntype:"error",msg:"Error saving Task Form",title:""})
},params:{formtype:k,action:"save",taskname:o,profile:ORYX.PROFILE,uuid:ORYX.UUID,tfvalue:i}})
}.bind(this)},{text:"Cancel",handler:function(){p.close();
p=null
}.bind(this)}],tbar:[b]});
p.show();
this.foldFunc=CodeMirror.newFoldFunction(CodeMirror.tagRangeFinder);
...
for(var d=0;
d<a.length;
d++){this.facade.offer({name:a[d],functionality:this.applyTheme.bind(this,a[d]),group:"colorpickergroup",dropDownGroupIcon:ORYX.BASE_FILE_PATH+"images/colorpicker.gif",icon:ORYX.BASE_FILE_PATH+"images/colorize.png",description:"Apply "+a[d]+" Color Theme",index:10,minShape:0,maxShape:0,isEnabled:function(){return true
}.bind(this)})
...
}else{this.facade.raiseEvent({type:ORYX.CONFIG.EVENT_NOTIFICATION_SHOW,ntype:"error",msg:"Invalid Color Theme data.",title:""})
}}catch(f){this.facade.raiseEvent({type:ORYX.CONFIG.EVENT_NOTIFICATION_SHOW,ntype:"error",msg:"Error applying Color Theme: "+f,title:""})
}}.bind(this),failure:function(){this.facade.raiseEvent({type:ORYX.CONFIG.EVENT_NOTIFICATION_SHOW,ntype:"error",msg:"Error applying Color Theme.",title:""})
...
this.facade.offer({name:"Lock",functionality:this.locknodes.bind(this),group:"lockunlockgroup",icon:ORYX.BASE_FILE_PATH+"images/lock.png",description:"Lock Elements",index:1,minShape:1,maxShape:0,isEnabled:function(){return true
}.bind(this)});
this.facade.offer({name:"Unlock",functionality:this.unlocknodes.bind(this),group:"lockunlockgroup",icon:ORYX.BASE_FILE_PATH+"images/unlock.png",description:"Unlock Elements",index:2,minShape:1,maxShape:0,isEnabled:function(){return true
}.bind(this)})
...
this.facade.offer({name:"Process Paths",functionality:this.findPaths.bind(this),group:"validationandsimulation",icon:ORYX.BASE_FILE_PATH+"images/path.png",dropDownGroupIcon:ORYX.BASE_FILE_PATH+"images/simulation.png",description:"Display Process Paths",index:1,minShape:0,maxShape:0,isEnabled:function(){return true
}.bind(this)});
this.facade.offer({name:"Run Simulation",functionality:this.runSimulation.bind(this),group:"validationandsimulation",icon:ORYX.BASE_FILE_PATH+"images/control_play.png",dropDownGroupIcon:ORYX.BASE_FILE_PATH+"images/simulation.png",description:"Run Process Simulation",index:2,minShape:0,maxShape:0,isEnabled:function(){return true
}.bind(this)});
...
this.facade.raiseEvent({type:ORYX.CONFIG.EVENT_NOTIFICATION_SHOW,ntype:"info",msg:"Creating path image.",title:""});
...
}else{this.facade.raiseEvent({type:ORYX.CONFIG.EVENT_NOTIFICATION_SHOW,ntype:"error",msg:"Invalid Path data.",title:""})
}}catch(g){this.facade.raiseEvent({type:ORYX.CONFIG.EVENT_NOTIFICATION_SHOW,ntype:"error",msg:"Error finding Paths:\n"+g,title:""})
}}.bind(this),failure:function(){this.facade.raiseEvent({type:ORYX.CONFIG.EVENT_NOTIFICATION_SHOW,ntype:"error",msg:"Error finding Paths.",title:""})
},params:{action:"getpathinfo",profile:ORYX.PROFILE,json:ORYX.EDITOR.getSerializedJSON(),ppdata:ORYX.PREPROCESSING,sel:""}})
}else{this.facade.raiseEvent({type:ORYX.CONFIG.EVENT_NOTIFICATION_SHOW,ntype:"error",msg:"Unknown path id.",title:""})
}},findPaths:function(){this.facade.raiseEvent({type:ORYX.CONFIG.EVENT_NOTIFICATION_SHOW,ntype:"info",msg:"Calculating process paths.",title:""});
var b=this.facade.getSelection();
var a="";
var c="Process Paths";
...
var j=Ext.id();
var d=new Ext.grid.EditorGridPanel({store:g,id:j,stripeRows:true,cm:new Ext.grid.ColumnModel([new Ext.grid.RowNumberer(),{id:"display",header:"Display Color",width:90,dataIndex:"display",renderer:function(e){if(e){return'<center><div width="20px" height="8px" style="width:20px;height:8px;background-color:'+e+'"></div></center>'
}else{return"<center>None</center>"
}}},{id:"numele",header:"Number of Elements",width:130,dataIndex:"numele",renderer:function(e){if(e){return"<center>"+e+"</center>"
}else{return"<center>0</center>"
}}}]),autoHeight:true});
var i=new Ext.Panel({id:"processPathsPanel",title:"<center>Select "+c+' and click "Show Path" to display it.</center>',layout:"column",items:[d],layoutConfig:{columns:1},defaults:{columnWidth:1}});
...
m.destroy()
}.bind(this)},buttons:[{text:"Show Path",handler:function(){if(d.getSelectionModel().getSelectedCell()!=null){var e=d.getSelectionModel().getSelectedCell()[0];
var t=g.getAt(e).data.pid;
this.setNodeColors(t,this.getDisplayColor(e),n[t])
}else{this.facade.raiseEvent({type:ORYX.CONFIG.EVENT_NOTIFICATION_SHOW,ntype:"info",msg:"Please select a process path.",title:""})
}}.bind(this)},{text:"Close",handler:function(){this.resetNodeColors();
m.hide()
}.bind(this)}]});
m.show()
}else{this.facade.raiseEvent({type:ORYX.CONFIG.EVENT_NOTIFICATION_SHOW,ntype:"error",msg:"Invalid Path data.",title:""})
}}catch(l){this.facade.raiseEvent({type:ORYX.CONFIG.EVENT_NOTIFICATION_SHOW,ntype:"error",msg:"Error finding Paths:\n"+l,title:""})
}}.bind(this),failure:function(){this.facade.raiseEvent({type:ORYX.CONFIG.EVENT_NOTIFICATION_SHOW,ntype:"error",msg:"Error finding Paths.",title:""})
...
}}}},runSimulation:function(){var a=new Ext.form.FormPanel({baseCls:"x-plain",labelWidth:150,defaultType:"numberfield",items:[{fieldLabel:"Number of instances",name:"instances",allowBlank:false,allowDecimals:false,minValue:1,width:120},{fieldLabel:"Interval",name:"interval",allowBlank:false,allowDecimals:false,minValue:1,width:120},{xtype:"combo",name:"intervalunits",store:new Ext.data.SimpleStore({fields:["units"],data:[["millisecond"],["seconds"],["minutes"],["hours"],["days"]]}),allowBlank:false,displayField:"units",valueField:"units",mode:"local",typeAhead:true,value:"minutes",triggerAction:"all",fieldLabel:"Interval units",width:120}]});
var b=new Ext.Window({autoCreate:true,layout:"fit",plain:true,bodyStyle:"padding:5px;",title:"Run Process Simulation",height:300,width:350,modal:true,fixedcenter:true,shadow:true,proxyDrag:true,resizable:true,items:[a],buttons:[{text:"Run Simulation",handler:function(){b.hide();
this.facade.raiseEvent({type:ORYX.CONFIG.EVENT_NOTIFICATION_SHOW,ntype:"info",msg:"Running Process Simulation...",title:""});
var c=a.items.items[0].getValue();
var e=a.items.items[1].getValue();
var d=a.items.items[2].getValue();
Ext.Ajax.request({url:ORYX.PATH+"simulation",method:"POST",success:function(f){try{if(f.responseText&&f.responseText.length>0&&f.responseText!="{}"){this.facade.raiseEvent({type:ORYX.CONFIG.EVENT_SIMULATION_SHOW_RESULTS,results:f.responseText})
}else{this.facade.raiseEvent({type:ORYX.CONFIG.EVENT_NOTIFICATION_SHOW,ntype:"info",msg:"Simulation engine did not return results.",title:""})
}}catch(g){this.facade.raiseEvent({type:ORYX.CONFIG.EVENT_NOTIFICATION_SHOW,ntype:"error",msg:"Unable to perform simulation:\n"+g,title:""})
}}.bind(this),failure:function(){this.facade.raiseEvent({type:ORYX.CONFIG.EVENT_NOTIFICATION_SHOW,ntype:"error",msg:"Unable to perform simulation.",title:""})
},params:{action:"runsimulation",profile:ORYX.PROFILE,json:ORYX.EDITOR.getSerializedJSON(),ppdata:ORYX.PREPROCESSING,numinstances:c,interval:e,intervalunit:d}})
}.bind(this)},{text:ORYX.I18N.FromBPMN2Support.close,handler:function(){b.hide()
}.bind(this)}]});
b.on("hide",function(){b.destroy(true);
delete b
...
var b='<table border="0" width="100%">                           <tr>                          <td><span style="font-size: 10px"><b>Process id: </b></span></td>                           <td><span style="font-size: 10px">'+a[0].id+'</span></td>                           </tr>                           <tr>                           <td><span style="font-size: 10px"><b>Process name: </b></span></td>                           <td><span style="font-size: 10px">'+a[0].name+'</span></td>                           </tr>                           <tr>                           <td><span style="font-size: 10px"><b>Process version: </b></span></td>                           <td><span style="font-size: 10px">'+a[0].version+'</span></td>                           </tr>                           <tr>                           <td><span style="font-size: 10px"><b>Simulation start: </b></span></td>                           <td><span style="font-size: 10px">'+a[0].starttime+'</span></td>                           </tr>                           <tr>                           <td><span style="font-size: 10px"><b>Simulation end: </b></span></td>                           <td><span style="font-size: 10px">'+a[0].endtime+'</span></td>                           </tr>                           <tr>                           <td><span style="font-size: 10px"><b>Num. of Executions: </b></span></td>                           <td><span style="font-size: 10px">'+a[0].executions+'</span></td>                           </tr>                           <tr>                           <td><span style="font-size: 10px"><b>Interval </b></span></td>                           <td><span style="font-size: 10px">'+a[0].interval+"</span></td>                           </tr>                           </table>";
if(a){Ext.getCmp("siminfopanel").body.update(b)
}},createGraphsTree:function(q){var p=new Ext.tree.TreeNode({});
var b;
var c;
this.resultsjson=q.results;
var k=jsonPath(q.results.evalJSON(),"$.processsim.*");
if(k){b=new Ext.tree.TreeNode({text:"Process",allowDrag:false,allowDrop:false,expanded:true,isLeaf:false,singleClickExpand:true});
c=new Ext.tree.TreeNode({id:"pgraph:processaverages",text:k[0].name+" ("+k[0].id+")",allowDrag:false,allowDrop:false,expanded:true,isLeaf:true,iconCls:"xnd-icon",icon:ORYX.BASE_FILE_PATH+"images/simulation/diagram.png",singleClickExpand:true});
b.appendChild(c);
p.appendChild(b)
}var m=jsonPath(q.results.evalJSON(),"$.htsim.*");
var g=jsonPath(q.results.evalJSON(),"$.tasksim.*");
if(m||g){b=new Ext.tree.TreeNode({text:"Activities",allowDrag:false,allowDrop:false,expanded:true,isLeaf:false,singleClickExpand:true});
for(var l=0;
l<m.length;
l++){var o=m[l];
...
if(e){b=new Ext.tree.TreeNode({text:"Paths",allowDrag:false,allowDrop:false,expanded:true,isLeaf:false,singleClickExpand:true});
for(var l=0;
l<e.length;
l++){var a=e[l];
...
ORYX.EDITOR.simulationChartTitle="Path Execution Info ("+c+")";
...
this.patternShapes[c.id].setProperty("oryx-name",c.name);
...
this.patternShapes[f.id].setProperty("oryx-name",f.name);
...
this.facade.offer({name:"Configure Snapshot Interval",functionality:this.configureSnapshotInterval.bind(this),group:"localstorage",icon:ORYX.BASE_FILE_PATH+"images/clock.png",dropDownGroupIcon:ORYX.BASE_FILE_PATH+"images/localhistory.png",description:"Configure Snaphot Interval",index:3,minShape:0,maxShape:0,isEnabled:function(){return ORYX.LOCAL_HISTORY_ENABLED
}.bind(this)});
...
}.bind(this)},buttons:[{text:"Restore",handler:function(){if(this.mygrid.getSelectionModel().getSelectedCell()!=null){var d=this.mygrid.getSelectionModel().getSelectedCell()[0];
...
var a=new Ext.Window({autoCreate:true,layout:"fit",plain:true,bodyStyle:"padding:5px;",title:"Configure Snapshot Interval",height:300,width:350,modal:true,fixedcenter:true,shadow:true,proxyDrag:true,resizable:true,items:[b],buttons:[{text:"Set",handler:function(){a.hide();
...
this.startStoring();
this.facade.raiseEvent({type:ORYX.CONFIG.EVENT_NOTIFICATION_SHOW,ntype:"info",msg:'Updated Snapshot Interval"',title:""})
}else{this.facade.raiseEvent({type:ORYX.CONFIG.EVENT_NOTIFICATION_SHOW,ntype:"error",msg:"Invalid input specified",title:""})
}}.bind(this)},{text:ORYX.I18N.FromBPMN2Support.close,handler:function(){a.hide()
}.bind(this)}]});
...
if(ORYX.PRESET_PERSPECTIVE!="ruleflow"){this.facade.offer({name:"Edit Process Form",functionality:this.editProcessForm.bind(this),group:"editprocessforms",icon:ORYX.BASE_FILE_PATH+"images/processforms.png",dropDownGroupIcon:ORYX.BASE_FILE_PATH+"images/processforms.png",description:"Edit Process Form",index:1,minShape:0,maxShape:0,isEnabled:function(){return true
}.bind(this)});
