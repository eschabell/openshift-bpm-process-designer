...
}return ORYX.PATH+"uuidRepository?uuid="+b+"&profile="+a+"&pp="+ORYX.PREPROCESSING+"&ts="+new Date().getTime()
};
...
}ORYX.Editor={DOMEventListeners:new Hash(),selection:[],zoomLevel:1,construct:function(c){this._eventsQueue=[];
this.loadedPlugins=[];
this.pluginsData=[];
...
this.modelMetaData=c;
var g=c;
if(c.model){g=c.model
}if(c.error){try{var e=new Ext.form.TextArea({id:"errorSourceTextArea",fieldLabel:"Error Details",value:c.error,autoScroll:true});
var d=new Ext.Panel({title:"Details",layout:"fit",border:false,items:[e],collapsed:true,listeners:{expand:function(n){this.foldFunc=CodeMirror.newFoldFunction(CodeMirror.tagRangeFinder);
var m=CodeMirror.fromTextArea(document.getElementById("errorSourceTextArea"),{mode:"text",lineNumbers:true,lineWrapping:true,onGutterClick:this.foldFunc})
}}});
var l=new Ext.Panel({title:"Info",layout:"table",border:false,bodyBorder:false,hideBorders:true,html:"<p/><p/><center>Could not open requested business process due to processing errors. <br/> Empty process was loaded instead.<br/> Click on the Details tab below to view error details.</center>"});
var b=new Ext.Panel({layout:"accordion",items:[l,d]});
var j=new Ext.Window({width:600,id:"processErrorSource",height:550,layout:"fit",title:"Process loading errors",items:[b],buttons:[{text:"Close",handler:function(){j.close();
j=null;
cf=null
}.bind(this)}]});
j.show()
}catch(f){alert(f)
}}this.id=g.resourceId;
if(!this.id){this.id=g.id;
if(!this.id){this.id=ORYX.Editor.provideId()
}}this.fullscreen=g.fullscreen||true;
this._initEventListener();
if(ORYX.CONFIG.BACKEND_SWITCH){var a=(g.stencilset.namespace||g.stencilset.url).replace("#","%23");
ORYX.Core.StencilSet.loadStencilSet(ORYX.CONFIG.STENCILSET_HANDLER+a,this.id)
}else{var a=g.stencilset.url;
ORYX.Core.StencilSet.loadStencilSet(a,this.id)
}if(!!ORYX.CONFIG.SSEXTS){ORYX.CONFIG.SSEXTS.each(function(m){this.loadSSExtension(m)
}.bind(this))
}this._createCanvas(g.stencil?g.stencil.id:null,g.properties);
this._generateGUI(c);
var h=false;
var k=false;
var i=function(){if(!h||!k){return
...
window.setTimeout(function(){this.loadSerialized(g);
...
this.simResultsTree=new Ext.tree.TreePanel({id:"simresultscharts",title:ORYX.I18N.View.sim.resultsGraphs,autoheight:true,animate:true,loader:new Ext.tree.TreeLoader(),rootVisible:false,autoScroll:false,lines:true,listeners:{click:{fn:this._chartSelected.bind(this)}}});
...
h.doLayout()
}.bind(this)},items:[{layout:"fit",title:ORYX.I18N.View.tabs.modelling,id:"processmodellingtab",items:[this.centerContentPanel]},{layout:"fit",title:ORYX.I18N.View.tabs.simResults,id:"simulationtab",autoScroll:false,items:[this.simResultsContentPanelLayout]}]};
...
for(region in this.layout_regions){if((region!="center"&&region!="north")&&ORYX.READONLY==true){this.layout_regions[region].setVisible(false)
