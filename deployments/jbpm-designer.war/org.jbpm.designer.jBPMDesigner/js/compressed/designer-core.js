...
}return ORYX.PATH+"uuidRepository?uuid="+b+"&profile="+a+"&pp="+ORYX.PREPROCESSING
};
ORYX.CONFIG.TRANSFORMER_URL=function(b,a){if(b===undefined){b=ORYX.UUID
}if(a===undefined){a=ORYX.PROFILE
}return ORYX.PATH+"transformer?uuid="+b+"&profile="+a
};
...
this.simInfoPanel=new Ext.Panel({bodyStyle:"background:#ffff;font-size:9px;font-family:Verdana, Geneva, Arial, Helvetica, sans-serif;padding-left:5px;",id:"siminfopanel",title:"Simulation Info",autoScroll:true,autoheight:true,border:false,html:""});
this.simResultsTree=new Ext.tree.TreePanel({id:"simresultscharts",title:"Simulation Graphs",autoheight:true,animate:true,loader:new Ext.tree.TreeLoader(),rootVisible:false,autoScroll:false,lines:true,listeners:{click:{fn:this._chartSelected.bind(this)}}});
...
h.doLayout()
}.bind(this)},items:[{layout:"fit",title:"Process Modelling",id:"processmodellingtab",items:[this.centerContentPanel]},{layout:"fit",title:"Simulation Results",id:"simulationtab",autoScroll:false,items:[this.simResultsContentPanelLayout]}]};
...
if(ORYX.READONLY==true&&b.region!="center"){}else{b.show()
...
this.facade.raiseEvent({type:ORYX.CONFIG.EVENT_NOTIFICATION_SHOW,ntype:"error",msg:"Failed to save process SVG.",title:""})
...
}catch(i){this.facade.raiseEvent({type:ORYX.CONFIG.EVENT_NOTIFICATION_SHOW,ntype:"error",msg:"Error: "+i,title:""});
var b=""
...
ORYX.Editor.checkIfSaved=function(){if(ORYX.READONLY==true){return true
...
}if(b.removestencils){$A(b.removestencils).each(function(e){delete this._availableStencils[b["extends"]+e]
}.bind(this))
...
}if(b.removestencils){$A(b.removestencils).each(function(d){var e=b["extends"]+d;
...
}},getSVGRepresentation:function(n){var i=this.getRootNode().cloneNode(true);
...
b=k.x+k.width;
l=k.y+k.height
}catch(j){this.getChildShapes(true).each(function(p){var r=p.absoluteBounds();
var q=r.upperLeft();
var e=r.lowerRight();
if(d==undefined){d=q.x;
m=q.y;
b=e.x;
l=e.y
}else{d=Math.min(d,q.x);
m=Math.min(m,q.y);
b=Math.max(b,e.x);
l=Math.max(l,e.y)
}})
...
}$A(i.getElementsByTagNameNS(ORYX.CONFIG.NAMESPACE_SVG,"image")).each(function(p){var e=p.getAttributeNS("http://www.w3.org/1999/xlink","href");
if(!e.match("^(http|https)://")&&e.indexOf("base64")==-1){e=window.location.protocol+"//"+window.location.host+e;
p.setAttributeNS("http://www.w3.org/1999/xlink","href",e)
}});
...
case ORYX.CONFIG.TYPE_INTEGER:var m=this._labels[g];
if(m){m.text(e)
}break;
...
case ORYX.CONFIG.TYPE_URL:case ORYX.CONFIG.TYPE_CALLEDELEMENT:var l=i.getAttributeNodeNS("","onclick");
...
},params:{profile:ORYX.PROFILE,uuid:ORYX.UUID,ppackage:k,pid:e,action:"openprocessintab"}})
...
var q=m.ownerDocument.createAttributeNS(null,"title");
q.nodeValue=this.getStencil().title();
n.setAttributeNode(q);
var t=m.ownerDocument.createAttributeNS(null,"id");
t.nodeValue=this.id;
n.setAttributeNode(t);
var b=this.node.childNodes[0].childNodes[0];
n=b.appendChild(n);
this.addEventHandlers(n);
var s=n.getAttributeNS(ORYX.CONFIG.NAMESPACE_ORYX,"minimumSize");
if(s){s=s.replace("/,/g"," ");
var j=s.split(" ");
j=j.without("");
...
if(i.isHorizontallyResizable){x.isHorizontallyResizable=true;
x.isResizable=true
}if(i.isVerticallyResizable){x.isVerticallyResizable=true;
x.isResizable=true
}if(i.anchorTop&&i.anchorBottom){x.isVerticallyResizable=true;
x.isResizable=true
}if(i.anchorLeft&&i.anchorRight){x.isHorizontallyResizable=true;
x.isResizable=true
}});
...
var x=this;
f.each(function(z){var D=new ORYX.Core.Controls.Magnet({eventHandlerCallback:x.eventHandlerCallback});
var y=parseFloat(z.getAttributeNS(ORYX.CONFIG.NAMESPACE_ORYX,"cx"));
var E=parseFloat(z.getAttributeNS(ORYX.CONFIG.NAMESPACE_ORYX,"cy"));
D.bounds.centerMoveTo({x:y-w,y:E-v});
var C=z.getAttributeNS(ORYX.CONFIG.NAMESPACE_ORYX,"anchors");
if(C){C=C.replace("/,/g"," ");
C=C.split(" ").without("");
for(var A=0;
A<C.length;
A++){switch(C[A].toLowerCase()){case"left":D.anchorLeft=true;
break;
case"right":D.anchorRight=true;
break;
case"top":D.anchorTop=true;
break;
case"bottom":D.anchorBottom=true;
break
}}}x.add(D);
if(!this._defaultMagnet){var B=z.getAttributeNS(ORYX.CONFIG.NAMESPACE_ORYX,"default");
if(B&&B.toLowerCase()==="yes"){x._defaultMagnet=D
}}})
}else{var h=new ORYX.Core.Controls.Magnet();
h.bounds.centerMoveTo(this.bounds.width()/2,this.bounds.height()/2);
this.add(h)
}var r=m.getElementsByTagNameNS(ORYX.CONFIG.NAMESPACE_ORYX,"docker");
if(r&&r.length>0){r=r[0];
var p=this.createDocker();
var e=parseFloat(r.getAttributeNS(ORYX.CONFIG.NAMESPACE_ORYX,"cx"));
var c=parseFloat(r.getAttributeNS(ORYX.CONFIG.NAMESPACE_ORYX,"cy"));
p.bounds.centerMoveTo({x:e-w,y:c-v});
var o=r.getAttributeNS(ORYX.CONFIG.NAMESPACE_ORYX,"anchors");
if(o){o=o.replace("/,/g"," ");
o=o.split(" ").without("");
for(var u=0;
u<o.length;
u++){switch(o[u].toLowerCase()){case"left":p.anchorLeft=true;
break;
...
$A(l).each((function(y){var i=new ORYX.Core.SVG.Label({textElement:y,shapeId:this.id});
i.x-=w;
i.y-=v;
this._labels[i.id]=i
}).bind(this))
...
var b=this.node.childNodes[0].childNodes[0];
b=ORYX.Editor.graft("http://www.w3.org/2000/svg",b,["g",{"pointer-events":"painted"}]);
