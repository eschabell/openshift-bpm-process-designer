...
}this.facade.offer({name:"EPC to BPMN transform",functionality:this.startTransform.bind(this),group:"epc",icon:ORYX.BASE_FILE_PATH+"images/epc_export.png",description:"Import from EPC",index:1,minShape:0,maxShape:0})
},startTransform:function(){this.showPanel(this.sendRequest.bind(this))
},sendRequest:function(c){var a=new Ext.Window({id:"oryx-loading-panel_epc2bpmn",bodyStyle:"padding: 8px",title:"Oryx",width:230,height:55,modal:true,resizable:false,closable:false,frame:true,html:'<span style="font-size:11px;">Please wait while importing...</span>'});
a.show();
if(!c||!c.url){return
...
}catch(f){Ext.Msg.alert(ORYX.I18N.Oryx.title,"An Error is occured while importing!")
...
var c={text:"Advanced Settings",xtype:"button",enableToggle:true,cls:"transform-epc-bpmn-group-button",handler:function(f){var f=Ext.getCmp("transform-epc-bpmn-id-advance");
if(f.collapsed){f.expand()
}else{f.collapse()
}}};
var a=new Ext.Window({title:ORYX.I18N.Oryx.title+" - Transform EPC to BPMN",width:400,id:"transform-epc-bpmn-id-panel",cls:"transform-epc-bpmn-window",items:new Ext.Panel({frame:true,autoHeight:true,items:[b,c,d]}),floating:true,shim:true,modal:true,resizable:false,autoHeight:true,buttons:[{text:"Import",handler:function(){var f={};
...
e(f)
}.bind(this)},{text:"Cancel",handler:function(){Ext.getCmp("transform-epc-bpmn-id-panel").close()
}}]});
a.show()
}});
